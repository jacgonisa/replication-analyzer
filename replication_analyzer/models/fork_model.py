"""
Fork detection models for replication analysis.
...
4-Class Model (Joint Fork + Origin Detection):
- Class 0: Background (no feature)
- Class 1: Left fork (leftward replication)
- Class 2: Right fork (rightward replication)
- Class 3: Origin (replication origin from curated annotations)
"""
...
def build_4class_fork_ori_model(max_length: int,
                                 n_channels: int = 9,
                                 n_classes: int = 4,
                                 cnn_filters: int = 64,
                                 lstm_units: int = 128,
                                 dropout_rate: float = 0.3) -> models.Model:
    """
    Build the 4-class joint fork and origin detection model.

    Architecture identical to 3-class fork model, but with 4-class softmax output
    to simultaneously detect left forks, right forks, and origins.
    ...
    n_channels : int
        Number of input channels (9 for enhanced encoding)
    n_classes : int
        Number of classes (4: background, left, right, origin)
    cnn_filters : int
        Number of CNN filters
    lstm_units : int
        Number of LSTM units
    ...
    -------
    tf.keras.Model
        4-class fork and origin detection model
    """
    inputs = layers.Input(shape=(max_length, n_channels), name='input')
    ...
    model = models.Model(inputs=inputs, outputs=outputs, name='Fork_Origin_Detector_4Class')
