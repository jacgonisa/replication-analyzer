"""
Training pipeline for 4-class joint fork and origin detection model.
"""
...
from ..data.preprocessing import prepare_4class_data_hybrid, pad_sequences, convert_xy_to_rectangular
from ..models.fork_model import build_4class_fork_ori_model
...
def train_4class_model(config):
    """
    Complete training pipeline for 4-class joint fork and origin detection.
    ...
    """
    print("\n" + "="*70)
    print("4-CLASS JOINT FORK + ORIGIN DETECTION MODEL TRAINING")
    print("="*70)
    ...
    # Optional: Convert to rectangular block representation
    use_rectangular = config['preprocessing'].get('use_rectangular_blocks', False)
    if use_rectangular:
        print(f"\n📊 Converting to rectangular block representation...")
        resolution_bp = config['preprocessing'].get('rectangular_resolution_bp', 500)
        print(f"   Resolution: {resolution_bp} bp")
        print(f"   Original bins: {len(xy_data):,}")
        xy_data = convert_xy_to_rectangular(xy_data, resolution_bp=resolution_bp)
        print(f"   Expanded blocks: {len(xy_data):,}")
        print(f"   ✅ Rectangular conversion complete")
    ...
    # Load origin annotations
    print(f"\n📍 Loading origin annotations...")
    ori_annotations = pd.read_csv(
        config['data']['ori_annotations_bed'],
        sep='\t',
        header=None,
        names=['chr', 'start', 'end', 'read_id', 'score', 'strand', 'chr2', 'read_start', 'read_end', 'run_id']
    )
    print(f"   Loaded {len(ori_annotations):,} origin annotations")
    ...
    X_sequences, y_sequences, info = prepare_4class_data_hybrid(
        xy_data,
        left_forks,
        right_forks,
        ori_annotations,
    ...
    n_ori = np.sum(y_train == 3)

    weight_background = total_segs / (4 * n_background) if n_background > 0 else 1.0
    weight_left = total_segs / (4 * n_left) if n_left > 0 else 1.0
    weight_right = total_segs / (4 * n_right) if n_right > 0 else 1.0
    weight_ori = total_segs / (4 * n_ori) if n_ori > 0 else 1.0
    ...
    print(f"   Origin:      {n_ori:,} ({n_ori/total_segs*100:.2f}%)")

    # ========== BUILD MODEL ==========
    print(f"\n🏗️  Building 4-class fork+origin model...")
    ...
    alpha = loss_config.get('alpha', [1.0, 2.0, 2.0, 2.0])

    # If alpha provided but needs normalization
    if isinstance(alpha, list) and len(alpha) == 4:
        class_weights = alpha
    else:
        class_weights = [weight_background, weight_left, weight_right, weight_ori]
    ...
    print(f"   Origin:      {class_weights[3]:.4f}")
    ...
    metrics = [
        'accuracy',
        MultiClassF1Score(n_classes=4, name='f1_macro')
    ]
...
def load_trained_4class_model(model_path, custom_objects=None):
    """
    Load a trained 4-class model.
    ...
    if custom_objects is None:
        from ..models.base import SelfAttention
        custom_objects = {
